...
export default function ResetPasswordOTPScreen({ route, navigation }) {
  ...
  const handleVerifyOTP = async () => {
    ...
    setLoading(true)
    try {
      console.log("🔍 Verificando código OTP para reset de contraseña...")
      ...
      navigation.navigate("NewPassword", { email, otpCode })
    } catch (error) {
      setLoading(false)
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      Alert.alert("✅ Código Reenviado", `Hemos enviado un nuevo código a:\n\n${email}`)
